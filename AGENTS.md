...
- 新建工作前搜索匹配的开放 PR、分支或 Issue，并原地继续唯一匹配项。普通单 PR 任务用 PR 正文维护动态状态；只有确实跨多个 PR、长期分阶段、需要独立积压或用户要求时才创建 Issue。
...
- 进度更新非阻断。有意义的里程碑按上述正式 checkpoint 流程执行，刷新 PR 当前已验证状态和下一步后继续，不要求用户回复“继续”。
