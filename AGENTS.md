...
以 GitHub 当前分支、SHA、PR、review、checks 为状态依据；有本地工作树时检查未知改动。已有匹配任务原地恢复，不创建替代 PR、不重做已验证工作。需要定位架构、命令或边界时查阅 `.github/CHATGPT_PROJECT_BRIEF.md`；续作时读取现有恢复状态，按当前决策所需读取相关 README/主题文档、PR/diff 和直接相关代码、测试、配置、workflow，再按影响或不确定性扩读。已读取且未变化的内容不重复加载，不默认加载全部技能、日志或历史。准确保留约束和证据身份。
...
中间失败、验收不通过、候选方案被否或假设失效，不等于任务结束。先定位原因，在原授权和任务预算（如有）内继续有证据支持的替代路径，或能区分不同原因的有限验证。尝试不再提供新信息时，重新评估其他已授权路线，不机械重复。达到验收即结束；确实无安全且已授权的动作可继续时，保存成果并报告具体阻塞或证据缺口，不要求证明研究目标绝对不可达。不得降低验收标准、隐藏失败证据或绕过安全、授权、冻结合同。
