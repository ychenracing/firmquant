...
anchor = '    ("doctor", "运行环境、身份与只读连接诊断"),\n'
smoke = '    ("smoke-readonly", "读取完整生产 authority surface 并持久化零写调用证据"),\n'
if smoke not in text:
    if text.count(anchor) != 1:
        raise SystemExit("CLI command-help anchor changed")
    text = text.replace(anchor, anchor + smoke, 1)
cli.write_text(text, encoding="utf-8")

checker = Path("scripts/check_docs.py")
...
if new not in text:
    if text.count(old) != 1:
        raise SystemExit("docs checker main anchor changed")
    text = text.replace(old, new, 1)
checker.write_text(text, encoding="utf-8")

Path("tests/unit/test_runtime_time_health_coverage.py").write_text(
...
def test_clock_guard_validates_configuration_and_builds_receipt() -> None:
    ...
    receipt = guard.verify(
        ClockObservation(NOW, NOW + timedelta(milliseconds=250), "Asia/Shanghai")
    )
    ...
    try:
        ...
        with lease.database.transaction("EXCLUSIVE"):
            lease.database.write(
                "UPDATE writer_leases SET expires_at = 'invalid' WHERE singleton_id = 1"
            )
