...
flowchart LR
    B[券商与行情事实] --> N[严格规范化]
    N --> A[uquant AccountState 同步语义]
    A --> U[ProductionEngine.decide]
    ...
    E --> W[单 writer 事务推进]
    W --> C[Reconciliation]
    C --> L[Operational Ledger / 报告 / 审计]
```

盘后决策和次日执行是两个独立、可恢复的 workflow step。盘中行情只供执行与安全检查，不能进入日频策略输入。
...
- `strategy`：锁定身份、uquant anti-corruption adapter、账户同步投影和 DecisionSnapshot。
...
- `reconciliation`：三类权威之间的资金、持仓、委托、成交和身份对账。
- `persistence`：SQLite migration、事务 repository、单 writer、恢复、备份和 hash-chain audit。
